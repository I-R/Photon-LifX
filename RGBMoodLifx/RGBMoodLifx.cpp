...
void RGBMoodLifx::tick() {
  ...
  if (pins_[0] >= 0) { // this is the modification for LIFX - allows 0 to be written for each pin to power off the LED
    ...
    RGB.control(true);
    RGB.color(current_RGB_color_[0], current_RGB_color_[1], current_RGB_color_[2]);
    //set red channel (8 bit value converter to 12 bit value)
    //pwm.setPWM(1, 0, (((current_RGB_color_[0])*16)));
    pwm.setPin(1, ((current_RGB_color_[0])*16), 0);
    //driver.getPin(PCA9685_LED0).setValueAndWrite(((current_RGB_color_[1]+1)*16)-1);
    //set green channel (8 bit value converter to 12 bit value)
    //pwm.setPWM(0, 0, (((current_RGB_color_[1])*16)));
    pwm.setPin(0, ((current_RGB_color_[1])*16), 0);
    //driver.getPin(PCA9685_LED1).setValueAndWrite(((current_RGB_color_[0]+1)*16)-1);
    //set bluw channel (8 bit value converter to 12 bit value)
    //pwm.setPWM(2, 0, (((current_RGB_color_[2])*16)));
    pwm.setPin(2, ((current_RGB_color_[2])*16), 0);
    //driver.getPin(PCA9685_LED2).setValueAndWrite(((current_RGB_color_[2]+1)*16)-1);
  }
}
